...
from models import LRGCN, LRGCN_v1
...
parser.add_argument("--w_d", type=float, default=1, help='Weight D loss')
parser.add_argument("--lambda_m", type=float, default=0.001, help='minfo hyperparam')


parser.add_argument("--ablation", type=int, default=0, help='ablation mode')
parser.add_argument("--lambda_f", type=float, default=0, help='Film hyperparam')
...
features, adj, tail_adj, gt, idx, labels = link_data_process.load_dataset(dataset)
...
# Model and optimizer
embed_model = model_used(nfeat=features.shape[1],
        nclass=nclass,
        params=args,
        device=device,
        ver=r_ver)
...
optimizer = optim.Adam(embed_model.parameters(),
                    lr=lr, weight_decay=args.w_decay)

if model_used == LRGCN_v1:
    feat_disc = args.hidden
else: 
    feat_disc = nclass
...
def train_embed(epoch, batch):
    ...
    if args.ablation == 4:
        L_all = loss_link + args.lambda_m * m_h 
        L_d = 0
    else:
        prob_h = disc(embed_h)
        prob_t = disc(embed_t)
        ...
        errorG = criterion(prob_t[batch], t_labels)
        L_d = errorG
        L_all = loss_link - (args.w_d * L_d) + args.lambda_m * m_h  
