...
from models import LRGCN_SP, LRGAT_SP, LRGSAGE_SP
...
parser.add_argument("--w_d", type=float, default=0.01, help='Weight D loss')
...
def train_embed(epoch):
    ...
    if args.ablation == 4 or args.ablation == 5:
        L_d = 0
    else:
        prob_h = disc(embed_h)
        prob_t = disc(embed_t)
...
features, adj, labels, idx = data_process.load_dataset(dataset)
...
# Model and optimizer
if args.arch == 1:
    embed_model = LRGCN_SP(nfeat=features.shape[1],
            nclass=nclass,
            params=args,
            device=device,
            ver=r_ver,
    ...
    embed_model = LRGAT_SP(nfeat=features.shape[1],
            nclass=nclass,
            params=args,
            device=device,
            ver=r_ver,
    ...
    embed_model = LRGSAGE_SP(nfeat=features.shape[1],
            nclass=nclass,
            params=args,
            device=device,
            ver=r_ver,
