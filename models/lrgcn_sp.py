...
from layers import GraphConv, Relation, Relationv2, Discriminator, Generator, SpGraphAttentionLayer

class RelGCN_SP(nn.Module):
    def __init__(self, nfeat, nhid, g_sigma, ver, ablation=0):
        super(RelGCN_SP, self).__init__()
...
class LRGCN_SP(nn.Module):
    def __init__(self, nfeat, nclass, params, device, ver=1, ablation=0):
        ...
        #self.rel1 = RelGCN_SP(nfeat, self.nhid, g_sigma=params.g_sigma, ver=ver)    
        if ver == 1:
        ...
        self.rel2 = RelGCN_SP(self.nhid, nclass, g_sigma=params.g_sigma, ver=ver, ablation=ablation).to(device)            
...
class LRGAT_SP(nn.Module):
    def __init__(self, nfeat, nclass, params, device, ver=1, ablation=0):
        super(LRGAT_SP, self).__init__()
        ...
        for i, attention in enumerate(self.gat1):
            self.add_module('attention_{}'.format(i), attention)
        self.rel2 = RelGAT_SP(nhid * nheads, nclass, g_sigma=params.g_sigma, device=device, ver=ver, ablation=ablation, nheads=1).to(device)
...
class LRGSAGE_SP(nn.Module):
    def __init__(self, nfeat, nclass, params, device, ver=1, ablation=0):
        super(LRGSAGE_SP, self).__init__()
